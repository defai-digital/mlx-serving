...
Based on mathematical modeling (see `automatosx/PRD/WEEK1-PERFORMANCE-MODELING.md`):
...
npx tsx benchmarks/flexible-benchmark.ts \
  ...
  --questions 100 \
  --output automatosx/tmp/baseline.json
```
...
npx tsx benchmarks/flexible-benchmark.ts \
  --model "mlx-community/Qwen2.5-7B-Instruct-4bit" \
  --questions 100 \
  --output automatosx/tmp/optimized.json
```
...
python scripts/compare-benchmarks.py \
  automatosx/tmp/baseline.json \
  automatosx/tmp/optimized.json
```
...
- **[Week 1 Blit Queue Implementation Summary](../automatosx/tmp/WEEK1-BLIT-QUEUE-IMPLEMENTATION-SUMMARY.md)** - Technical implementation details
...
- `automatosx/PRD/WEEK1-PERFORMANCE-MODELING.md` - Performance targets
