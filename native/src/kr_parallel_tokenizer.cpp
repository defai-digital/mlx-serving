...
ParallelTokenizer::ParallelTokenizer(const ParallelTokenizerConfig& config)
    ...
    active_threads_.store(config_.num_threads);
#else
    active_threads_.store(1);
#endif
}
...
void ParallelTokenizer::accelerateStringOp(const char* src, char* dst, size_t len) const {
    ...
    if (config_.use_accelerate) {
        ...
        if (config_.enable_stats) {
            accelerate_ops_.fetch_add(1, std::memory_order_relaxed);
        }
        return;
    }
#endif
...
void ParallelTokenizer::recordTiming(uint64_t start_time_us, size_t num_tokens, size_t num_bytes) {
    ...
    total_encode_time_us_.fetch_add(elapsed_us, std::memory_order_relaxed);
    total_tokens_.fetch_add(num_tokens, std::memory_order_relaxed);
    total_bytes_.fetch_add(num_bytes, std::memory_order_relaxed);
}
...
std::vector<uint32_t> ParallelTokenizer::encode(
    ...
    if (text.size() < config_.min_chunk_size * config_.num_threads) {
        ...
        if (config_.enable_stats) {
            total_encodes_.fetch_add(1, std::memory_order_relaxed);
    ...
    if (config_.enable_stats) {
        ...
        speedup_ratio_.store(speedup, std::memory_order_relaxed);
    }
#else
...
std::vector<std::vector<uint32_t>> ParallelTokenizer::encodeBatch(
    ...
    if (texts.size() <= 2) {
        ...
        if (config_.enable_stats) {
            total_batch_encodes_.fetch_add(1, std::memory_order_relaxed);
...
ParallelTokenizerStatistics ParallelTokenizer::getStatistics() const {
    return {
        .total_encodes = total_encodes_.load(),
        .total_batch_encodes = total_batch_encodes_.load(),
        .total_tokens = total_tokens_.load(),
        .total_bytes = total_bytes_.load(),
        .total_encode_time_us = total_encode_time_us_.load(),
        .speedup_ratio = speedup_ratio_.load(),
        .active_threads = active_threads_.load(),
        .accelerate_ops = accelerate_ops_.load()
    };
}

// Reset statistics
void ParallelTokenizer::resetStatistics() {
    total_encodes_.store(0);
    total_batch_encodes_.store(0);
    total_tokens_.store(0);
    total_bytes_.store(0);
    total_encode_time_us_.store(0);
    speedup_ratio_.store(1.0);
    accelerate_ops_.store(0);
}
