 ...
 * Run: npx tsx benchmarks/compare-vision-fair.ts [config.yaml]
 *
 * Configuration is read from YAML file (default: benchmarks/compare-vision-fair.yaml)
 */
...
function loadConfig(configPath: string = 'benchmarks/compare-vision-fair.yaml'): BenchmarkConfig {
  try {
    const configFile = readFileSync(configPath, 'utf8');
    const config = YAML.parse(configFile) as BenchmarkConfig;

    // Validate configuration
    if (!config.benchmark || !config.models || !config.prompts || !config.test_images) {
    ...
    if (config.prompts.length === 0) {
      throw new Error('No prompts defined in configuration');
    }

    if (config.test_images.length === 0) {
      throw new Error('No test images defined in configuration');
    }
...
let TEMP: number;
let SAMPLE_PROMPTS: string[];
let TEST_IMAGES: string[];
...
class MLXEngineVisionServer {
  ...
  private static readonly MODEL_LOAD_TIMEOUT_MS = 600_000;  // 600 seconds (10 minutes for very large vision models)
  private static readonly PYTHON_VENV_PATH = '.mlx-engine-venv/bin/python';
  private static readonly SERVER_SCRIPT = 'benchmarks/mlx-engine-vision-server.py';
  ...
  private validateStartPreconditions(): void {
    ...
    if (this.isStarting) {
      throw new Error(MLXEngineVisionServer.ERROR_MESSAGES.START_IN_PROGRESS);
    }

    // Bug #19 fix: Prevent starting while already running
    // Refactoring #2: Use constant for error message
    if (this.process && !this.processFailed) {
      throw new Error(MLXEngineVisionServer.ERROR_MESSAGES.ALREADY_RUNNING);
    }
  }
  ...
  private spawnPythonProcess(): void {
    ...
    this.process = spawn(MLXEngineVisionServer.PYTHON_VENV_PATH, [MLXEngineVisionServer.SERVER_SCRIPT]);
  }
  ...
  private validateProcessStreams(): void {
    ...
    if (!this.process?.stdout || !this.process?.stdin) {
      throw new Error(MLXEngineVisionServer.ERROR_MESSAGES.STREAM_VALIDATION_FAILED);
    }
  }
  ...
  private async waitForModelLoad(model: string): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      this.stderrListener = (data: Buffer) => {
        ...
        if (message.includes(MLXEngineVisionServer.MODEL_LOAD_STDERR_MARKER)) {
          resolved = true;
      ...
      this.modelLoadTimeoutId = setTimeout(() => {
        if (!resolved) {
          ...
          this.modelLoadReject = null;  // Bug #9 fix: Clear reject reference
          this.modelLoadTimeoutId = null;
          reject(new Error(MLXEngineVisionServer.ERROR_MESSAGES.MODEL_LOAD_TIMEOUT));
        }
      }, MLXEngineVisionServer.MODEL_LOAD_TIMEOUT_MS);
    });
   ...
   *
   * @param model - Model identifier to load (e.g., "mlx-community/Qwen2.5-VL-7B-Instruct-4bit")
  ...
  /**
   * Generates text using the loaded vision model with image input
   *
   ...
   * @param imagePath - Path to the image file for vision model
  ...
  async generate(prompt: string, imagePath: string, maxTokens: number, temp: number): Promise<{ tokens: number }> {
    ...
    // Refactoring #2: Use constant for error message
    if (!process || !stdin) {
      throw new Error(MLXEngineVisionServer.ERROR_MESSAGES.SERVER_NOT_STARTED);
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeoutId = setTimeout(() => {
        ...
        if (pending) {
          ...
          reject(new Error(MLXEngineVisionServer.ERROR_MESSAGES.REQUEST_TIMEOUT));
        }
      }, REQUEST_TIMEOUT); // Timeout from config
  ...
  private cleanupModelLoadPromise(error?: Error): void {
    ...
    if (this.modelLoadReject) {
      this.modelLoadReject(error || new Error(MLXEngineVisionServer.ERROR_MESSAGES.SERVER_STOPPED_DURING_LOAD));
      this.modelLoadReject = null;
    }
  ...
  async stop(): Promise<void> {
    ...
    // Refactoring #4: Use extracted cleanup methods
    this.cleanupModelLoadPromise(new Error(MLXEngineVisionServer.ERROR_MESSAGES.SERVER_STOPPED_DURING_LOAD));
    this.rejectAllPendingRequests(new Error(MLXEngineVisionServer.ERROR_MESSAGES.SERVER_STOPPED));
...
async function benchmarkMLXEngine(cycle: number, server: MLXEngineVisionServer, questionCount: number): Promise<BenchmarkResult> {
...
async function main() {
  ...
  const configPath = process.argv[2] || 'benchmarks/compare-vision-fair.yaml';
  ...
  try {
    ...
    console.error('\nUsage: npx tsx benchmarks/compare-vision-fair.ts [config.yaml]');
    process.exit(1);
  }
  ...
  console.log('Fair Vision Benchmark - mlx-vlm vs mlx-serving');
  ...
  for (const modelConfig of enabledModels) {
    ...
    // Print results
    console.log('\n\n' + '='.repeat(60));
    console.log(`VISION BENCHMARK RESULTS: ${MODEL}`);
    console.log('='.repeat(60));
  ...
  console.log('\n\n' + '✅'.repeat(30));
  console.log(`🎉 ALL ${enabledModels.length} VISION BENCHMARK${enabledModels.length > 1 ? 'S' : ''} COMPLETED!`);
  console.log('✅'.repeat(30));
}
